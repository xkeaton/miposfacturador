...
<script>
    ...
    $(document).ready(function() {
        ...
        $("#id_almacen_venta").change(function() {
            var table = $('#tbl_ListadoProductos').DataTable();
            if (table.rows().count() > 0) {
                Swal.fire({
                    title: '¿Está seguro de cambiar de almacén?',
                    text: "Se limpiará el listado de productos agregados.",
                    icon: 'warning',
                    ...
                    if (result.isConfirmed) {
                        ...
                        $("#id_almacen_venta").val($("#id_almacen_venta").data('prev'));
                    }
                });
            } else {
                $("#id_almacen_venta").data('prev', $(this).val());
    ...
    function fnc_CargarAutocompleteProductos() {
        ...
        $("#producto").autocomplete({
            source: function(request, response) {
                $.ajax({
                    url: "ajax/autocomplete_productos.ajax.php",
                    dataType: "json",
                    data: {
                        term: request.term,
        ...
        if ($("#forma_pago").val() == 2) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    window.open($ruta+'vistas/modulos/impresiones/generar_nota_venta_ticket.php?id_venta=' + response["id_venta"],
                        "ModalPopUp",
                        "toolbar=no," +
                        "scrollbars=no," +
                        "location=no," +
